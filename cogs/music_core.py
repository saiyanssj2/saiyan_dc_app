...
import sys as _sys
if _COOKIES_OPTS:
    print(f"[cookies] Loaded from: {_COOKIES_PATH} ({os.path.getsize(_COOKIES_PATH)} bytes)", flush=True)
else:
    print(f"[cookies] NO COOKIES FOUND! Path={_COOKIES_PATH}, env={'set' if _COOKIES_ENV else 'unset'}", file=_sys.stderr, flush=True)
