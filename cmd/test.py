...
def setup(bot):
    ...
    async def play(interaction: discord.Interaction, url: str):
        ...
        if user.voice:
            ...
            if "entries" in data:
                info = [{"title": entry["title"], "url": entry["url"]} for entry in data["entries"] if entry.get("channel_id")]
            else:
                info = [{"title": data["title"], "url": data["url"]}]
                
            url2 = info[index]['url']  # Lấy URL stream

            # Thêm bài hát vào hàng đợi
            music_queue.add((url2, info[index]['title']))
            ...
            if not discord.utils.get(bot.voice_clients, guild=interaction.guild):
                ...
                await interaction.followup.send(f"Đang phát nhạc từ: {info[index]['title']}")
            else:
                await interaction.followup.send(f"Đã thêm vào hàng đợi: {info[index]['title']}")
        else:
